...
#' @author Gaye A.
#'
glm.analysis.GxG <- function(pheno.model=NULL, observed.data=NULL){

  # BINARY OUTCOME
  if(pheno.model == 0){
	   # FIT CONVENTIONAL UNCONDITIONAL LOGISTIC REGRESSION MODEL
	   mod.glm <- glm(phenotype ~ 1+genotype1+genotype2+interaction,family=binomial,data=observed.data)
  ...
  if(pheno.model == 1){
	    ...
	    mod.glm <- glm(phenotype ~ 1+genotype1+genotype2+interaction,family=gaussian,data=observed.data)
	    mod.sum <- summary(mod.glm)     
  }
